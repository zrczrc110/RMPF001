    ...
    int idx)
{
  ...
  if (ret) {
    pr_err("sswap_rdma_wait_for_cm failed cm_error:%d ret:%d\n", queue->cm_error, ret);
	rdma_destroy_id(queue->cm_id);
	pr_info("after rdma_destroy_id");
    return ret;
    //goto out_destroy_cm_id;
  }
...
out_destroy_cm_id:
  pr_err("before out_destroy_cm_id");
  rdma_destroy_id(queue->cm_id);
  pr_err("after out_destroy_cm_id");
  return ret;
}
